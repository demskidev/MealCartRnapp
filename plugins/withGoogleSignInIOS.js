const { withAppDelegate, withPodfile, withInfoPlist } = require('@expo/config-plugins');

function withGoogleSignInIOS(config) {
  // Add URL schemes to Info.plist for Google Sign-In callback
  config = withInfoPlist(config, (config) => {
    const googleUrlScheme = 'com.googleusercontent.apps.107165390600-sni5oc9le9cnucc89mqv7e51eq0undge';
    ...
    const hasGoogleScheme = config.modResults.CFBundleURLTypes.some(
      (type) => type.CFBundleURLSchemes && type.CFBundleURLSchemes.includes(googleUrlScheme)
    );
    ...
    const bundleId = config.ios?.bundleIdentifier || 'com.jbehl23.mealcart';
    const hasBundleScheme = config.modResults.CFBundleURLTypes.some(
      (type) => type.CFBundleURLSchemes && type.CFBundleURLSchemes.includes(bundleId)
    );
  ...
  config = withAppDelegate(config, (config) => {
    ...
    const hasApplicationOpenMethod = /public\s+override\s+func\s+application\s*\(\s*_\s+app:\s*UIApplication,\s*open\s+url:\s*URL/.test(contents);
